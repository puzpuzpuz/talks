...
![bg right:33% w:200 h:200](./images/hazelcast-logo.png)
...
* Сначала пробуем PoC с полумерой
...
1. Эксперимент с пулом буферов для сериализации
    - Неудачный. К тому же, `Buffer#allocUnsafe()` и так [использует](https://nodejs.org/api/buffer.html#buffer_class_method_buffer_allocunsafeslow_size) пул<br/>(8 KB by default)
...
* В блокирующихся клиентских библиотеках схожий прием называют [pipelining](http://informatikr.com/2012/redis-pipelining.html)
* Пример pipelining API в нашем Java-клиенте:
...
* *Примечание*: данная оптимизация не отменяет появление отдельного batch API в будущем
...
* *Спойлер*: заметной прибавки в нашем бенчмарке минорные оптимизации не дали, но они все равно полезны
